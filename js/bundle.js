...
function ReportsTab({ history, prices, onToggleConcretada }){
  ...
  const exportCSV = () => {
    const header = [
      "Fecha","Cliente","Agendó","Atendió","Tramo","Gramos Total","G. Cadena","G. Micro","G. Italiana","G. GF 18K",
      "Venta","Costo","Ganancia","Comisión Agendó (35%)","Comisión Atendió (35%)","Comisión Empresa (30%)"
    ];
    const catToGroup = {
    ...
    const rows = monthQuotes.map(q => {
      ...
      return [
        ...
        (Math.round((q.totalWeight||0)*10)/10).toLocaleString('es-CL',{maximumFractionDigits:1}),
        (Math.round((gramsByGroup.cadena||0)*10)/10).toLocaleString('es-CL',{maximumFractionDigits:1}),
        (Math.round((gramsByGroup.micro||0)*10)/10).toLocaleString('es-CL',{maximumFractionDigits:1}),
        (Math.round((gramsByGroup.italiana||0)*10)/10).toLocaleString('es-CL',{maximumFractionDigits:1}),
        (Math.round((gramsByGroup.gf18k||0)*10)/10).toLocaleString('es-CL',{maximumFractionDigits:1}),
        total,
